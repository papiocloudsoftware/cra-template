...
async function verify<T extends TokenPayload>(
  jwt: string,
  excludeExp = false
): Promise<T> {
...
async function logout() {
  document.cookie = `${refreshTokenCookieName}=; domain=localhost; expires=${new Date(
    0
  )};`;
  return {};
}

async function authTokensResponse(
...
async function refreshToken(
  ...
  try {
    ...
    if (accessToken && refreshTokenString) {
      const refreshToken: RefreshTokenPayload = await verify(
        refreshTokenString
      );
